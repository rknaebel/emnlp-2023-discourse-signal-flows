...
from helpers.crf import SignalLabelDataset, RobertaWithCRF, decode_labels, decode_labels_gold
from helpers.data import get_corpus_path, load_docs
from helpers.evaluate import score_paragraphs


def train_model_labeling(model, relation_type, train_dataloader, eval_dataloader,
                         save_path="", validation_metric="f1-score", majority_class_weight=1.0):
    optimizer = AdamW([
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(eval_dataloader, total=len(eval_dataloader), mininterval=5, desc='Eval'):
            ...
            for pred, ref in zip(preds.tolist(), batch['labels'].tolist()):
                pred = [model.config.id2label[p] for i, p in enumerate(pred) if ref[i] != 7]
                ref = [model.config.id2label[i] for i in ref if i != 7]
                ...
                signals_gold.append(
                    [(relation_type, [i for p, i in signal], None) for signal in decode_labels_gold(ref, ref)])
            metric.add_batch(predictions=predictions, references=references)
        loss_valid = np.mean(losses)

        print("Logits:", logits)
        print("ArgMax:", preds)
        print("GOLD:", signals_gold[:10])
        print("PRED:", signals_pred[:10])
        ...
        for key, vals in results.items():
            if key == 'accuracy':
                print(f"{key:15}  {vals * 100:6.2f}")
            else:
                print(
                    f"{key:15}  {vals['precision'] * 100:6.2f}  {vals['recall'] * 100:6.2f}  {vals['f1-score'] * 100:6.2f}  {vals['support']:7d}")

        print(f'Training loss: {loss_train}')
        print(f'Validation loss: {loss_valid}')
        current_score = results['macro avg'][validation_metric]
...
def main(corpus, relation_type, batch_size, split_ratio, save_path, test_set, random_seed, majority_class_weight,
    ...
    best_model_state = train_model_labeling(model, relation_type, train_dataloader, eval_dataloader, save_path,
                                            val_metric, majority_class_weight)
    # optimizer = AdamW([
    #     {'params': model.roberta.parameters(), 'lr': 5e-5},
    #     {'params': model.classifier.parameters()}
    # ], lr=1e-3)
    ...
    #     print(f'Training loss: {loss_train}')
    #     print(f'Validation loss: {loss_valid}')
    #     current_score = results['macro avg'][val_metric]
    #     if current_score > best_score:
    #         best_score = current_score
    #         print(f"Store new best model! Score: {current_score}...")
    #         model_save_path = os.path.join(save_path, f"best_model_{relation_type.lower()}_label")
